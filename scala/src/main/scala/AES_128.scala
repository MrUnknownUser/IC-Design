...
class AesIterative extends Component {
  ...
  // Helpers encrypt: mapping (column-major)
  ...
  when(io.start && !running && !io.decrypt) {
    ...
    val rkBitsUsed = matrixToBits(keyWordsToMatrix(prevRoundKey))
    val afterAddBits = matrixToBits(invSub) ^ rkBitsUsed 
    val afterAddMat = bitsToMatrix(afterAddBits)
